...
uint32_t charLength(const char *byte);
uint32_t byteLength(const char *byte);
int len_utf8(const unsigned char byte);
uint32_t max_occurance(const char *byte);

uint32_t charLength(const char *byte){ //assumes the utf8 string is valid
	int len = 0;
...
uint32_t byteLength(const char *byte){ //assumes the utf8 string is valid
	int len = 0;
